...
## The Planner's own guardrails

Everything above is the shared, family-wide safety model. This section is
what The Planner adds on top of it.

### What it will never do

- **Publish a rota without an explicit approval.** `Generate` (the weekly
  build) only ever produces a `draft`/`needs_human` item; `schedule_shifts`
  is only ever written by `tools/scheduling.py:dispatch_weekly_rota`, and
  that function refuses to run unless the item has been approved or edited
  first (`core.review.assert_write_allowed`, action `publish`) - see
  docs/how-it-works.md.
- **Loosen a working-time limit to fill a gap.** `max_consecutive_days`,
  `min_rest_hours` and the quota headroom floor
  (`config/agent.yaml: working_time`) are enforced *unconditionally* by
  excluding a person from the candidate pool - never gated by a rule
  toggle, and never by a post-hoc check that could be argued around.
  `rules.quota-hard-cap` is a separate, *soft* preference among candidates
  already above that floor (see docs/how-it-works.md "Design decisions") -
  turning it off changes who is picked first, never who is excluded. A
  floor or a service that comes back short-staffed is reported as a
  warning, never silently filled by breaking a limit.
- **Reassign a shift without respecting the same rules the weekly rota
  used.** A swap or sick-day request is matched against the same
  personal-rules/quota/working-time checks as the original plan
  (`tools/engine.py:resolve_swap`) - it will never offer a shift to someone
  who was excluded from it in the first place.
- **Invent a warning, a name or a number in a narrative.** Both LLM prompts
  (`prompts/duty-manager-briefing.md`, `prompts/staff-brief.md`) say
  explicitly to use only the facts in the JSON they are given. The
  rota itself, the swap match, and every warning are computed by
  `tools/engine.py` before either prompt ever runs - the model writes the
  words, never the numbers.

### Escalation - queued as `needs_human`

- **A weekly rota** whenever any day in the week has a warning - a short
  floor team, a missing supervisor, an unfilled sommelier slot, an
  acting-lead promotion. See docs/how-it-works.md "Deciding what needs a
  human" for the full list.
- **A swap or sick-day request** whenever no eligible replacement exists
  for that role, department and day.
- **An unrecognised staff id** on a swap/sick-day request (a typo forwarded
  from a WhatsApp message, for example) - `tools/engine.py:resolve_swap`
  reports it plainly rather than guessing who was meant.

### Data handling specific to this agent

- **Staff data, not guest data.** `fixtures/hotel/staff.json` /
  `data/imports/staff.csv` hold your team's contact details, contracted
  hours and pay rate - treat this file with the same care as any HR export.
  It never leaves the machine except as the prompt context for the two LLM
  calls above (with `llm.provider: anthropic` or `claude-code`), and never
  in the drafts themselves (neither prompt asks for or echoes an hourly
  rate).
- **No card or payment data passes through this agent at all** - there is
  nothing to redact here that `core.redact` was built for, but the guard
  still runs on every inbound swap/sick-day note as a matter of course.
